...
\item The company has very little in-house IT capacity, and does not
  ...
  its own servers.
\item Since the backend servers must consequently be outsourced, the
  server software has to run in a generic, non-customised environment
  (such as a standard Linux server).
...
\item Trucking in Russia is at an unusually high risk of hijacking.
  ...
  communications has to be encrypted, and all queries in the database
  must be authorised.
\end{itemize}
